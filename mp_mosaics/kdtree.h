...
class KDTree
{
  ...
  private:
     ...
     */
     int partition(vector<Point<Dim>>& points, int a, int b, int currDim);
     int quickselect(vector<Point<Dim>>& points, int a, int b, int k, int currDim);
     KDTreeNode* construct(vector<Point<Dim>>& points, int a, int b, int currDim);
     KDTreeNode* copy(KDTreeNode* subRoot);
     void destroy(KDTreeNode* subRoot);
     Point<Dim> findhelper(const Point<Dim>& query, KDTreeNode* root,int currDim) const;
     int distance(const Point<Dim>& init, const Point<Dim>& target) const;
};
